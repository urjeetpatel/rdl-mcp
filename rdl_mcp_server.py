...
class MCPServer:
    ...
    def _extract_field_references_with_context(self, expression: str, default_dataset: str) -> Dict[str, List[str]]:
        ...
        handled_positions: set = set()  # Track field positions we've already assigned to a dataset

        # 1. Handle Lookup/LookupSet/MultiLookup functions
        # Pattern: Lookup(source_expr, dest_expr, result_expr, "DatasetName")
        # The source_expr uses the current dataset, dest_expr and result_expr use the named dataset
        ...
        # 2. Handle aggregate functions with dataset scope: Sum/Count/First/Last/Min/Max/Avg(expr, "DatasetName")
        # These aggregates can take a second parameter specifying the dataset scope
        ...
        for match in re.finditer(aggregate_pattern, expression, re.IGNORECASE):
            ...
            scope_dataset = match.group(3)  # May be None if no scope specified

            # Determine which dataset these fields belong to
            ...
            for field_match in re.finditer(r'Fields!(\w+)', field_expr):
                ...
                # Skip if already handled by Lookup
    ...
    def _extract_field_references(self, expression: str) -> List[str]:
        """Extract all field names referenced in an RDL expression (legacy method).

        Returns all field names without dataset context.
        """
        import re
        if not expression or not expression.strip().startswith('='):
            return []
    ...
    def validate_rdl(self, filepath: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for dataset in datasets:
                ...
                # Collect field names for this dataset
                field_names = set()
                for field in dataset.findall(f'.//{ns}Field'):
                    field_name = field.get('Name')
                    if field_name:
            ...
            for tablix in tablixes:
                ...
                def validate_expression(expression: str, location: str):
                    """Validate field references in an expression, respecting Lookup dataset context."""
                    if not expression:
                        return

                    # Extract fields with their dataset context
                    fields_by_dataset = self._extract_field_references_with_context(expression, dataset_name)

                    for ref_dataset, fields in fields_by_dataset.items():
                        # Get valid fields for this dataset
                        if ref_dataset not in dataset_fields:
                            # Dataset doesn't exist - report error for each field
                ...
                # Track which Value elements we've already processed in specific contexts
                processed_value_ids = set()

                # First check GroupExpressions (these have direct text, not Value children)
                for group_expr in tablix.findall(f'.//{ns}GroupExpression'):
                    validate_expression(group_expr.text, 'GroupExpression')

                # Check SortExpressions and track their Value elements
                ...
                # Find all Value elements in the Tablix that contain expressions
                # Skip ones we've already processed in specific contexts
                for value_elem in tablix.findall(f'.//{ns}Value'):
                    ...
                    if expression and expression.strip().startswith('='):
                        # Try to find which textbox contains this expression for better error messages
                        parent = value_elem
                        textbox_name = None
                        for _ in range(10):  # Walk up max 10 levels
                            parent = self._find_parent(tablix, parent)
                            if parent is None:
                                break
                ...
                # Deduplicate invalid refs by (field, dataset) combination
                ...
                for ref in unique_invalid_refs:
                    ...
                    # Get available fields for the referenced dataset
                    ...
                    # Check if this is a special error (like unknown dataset)
                    if ref.get('error'):
                        issues.append(
